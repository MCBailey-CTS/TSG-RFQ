...
namespace RFQ
{
    public partial class NCEditQuote : System.Web.UI.Page
    {
        ...
        protected void populateGeneralNotes()
        {
                ...
                sql.CommandText = "Select count(*) from linkGeneralNoteToQuoteNc where gnqncQuoteID = @quoteID";
                sql.Parameters.AddWithValue("@quoteID", quoteID);
                ...
                if (!notes)
                {
                    ...
                    {
                        sql.Parameters.Clear();
                        sql.CommandText = "Insert into linkGeneralNoteToQuoteNc (gnqGeneralNoteID, gnqncQuoteID, gnqncCreated, gnqncCreatedBy) ";
                ...
                sql.CommandText = "Select gnoGeneralNote as GeneralNote, (Select count(gnqncGeneralNoteID) from linkGeneralNoteToQuoteNc where gnqncQuoteID = @quoteID and gnqncGeneralNoteID = gnoGeneralNoteID) as num, gnoGeneralNoteID as ids from pktblGeneralNote where gnoCompany = @company";
                sql.Parameters.AddWithValue("@quoteID", quoteID);
                sql.Parameters.AddWithValue("@company", "NewCo");
        ...
        protected void saveGeneralNotes_Click(object sender, EventArgs e)
        {
            ...
            sql.CommandText = "Delete from linkGeneralNoteToQuoteNC where gnqQuoteID = @quoteID";
            sql.Parameters.AddWithValue("@quoteID", quoteID);
            ...
            int k = 0;
                ...
                foreach (GridViewRow row in dgGeneralNotes.Rows)
                {
                    ...
                    {
                        sql.Parameters.Clear();
                        sql.CommandText = "Insert into linkGeneralNoteToQuoteNC (gnqGeneralNoteID, gnqQuoteID, gnqCreated, gnqCreatedBy) ";
                        sql.CommandText += "Values (@noteID, @quoteID, GETDATE(), @created)";
        ...
        protected void populate_Header()
        {
            ...
            populateGeneralNotes();
                ...
                sql.CommandText = "Select ncqquoteID, ncqQuotationNumber, ncqQuotationType, ncqCustomerRFQ, ncqDate, ncqJobNumber, ncqCustomer, ncqPlant, ";
                sql.CommandText += "ncqCustomerContact, ncqEstimatorID, ncqVersion, ncqQuoteProcess, ncqPartNumber, ncqProgramName, ncqProgramKickOffDate,  ";
                sql.CommandText += "TSGSalesman.Name, ncqProgramPPAPDate, ncqProgramSOPDate, ncqYearsOfProduction, ncqAnnualVolume, ncqQuotedLotSize,  ";
                sql.CommandText += "ncqDataRecievedDate, ncqDataFileName, ncqDesignReview, ncqTryout, ncqShippingCompany, ncqSetup, ncqBuyoff,  ";
                sql.CommandText += "ncqPaymentTermsToolingEquipmentCapital,  ncqPaymentTermsPieceCost, ncqRawMaterial, ncqWIP, ncqFinnishedGoods, ";
                sql.CommandText += "ncqShippingOfFinishedGoods, ncqTotalToolingCost, ncqTotalCapitalCost, ncqTotalPieceCost, ncqCreatedBy ";
                sql.CommandText += "from Customer, CustomerLocation, TSGSalesman, tblncquote  ";
                sql.CommandText += "where ncqquoteID = @quoteID and ncqCustomer = Customer.CustomerID and ncqPlant = CustomerLocationID and TSGSalesman.TSGSalesmanID = CustomerLocation.TSGSalesmanID ";
                ...
                if (dr.Read())
                {
                    try
                    {
                        ddlCustomer.SelectedValue = dr["ncqCustomer"].ToString();
                        cust = dr["ncqCustomer"].ToString();
                        if (ddlCustomer.SelectedValue == cust)
                        {
                            populate_Plants();
                        }
                        ddlPlant.SelectedValue = dr["ncqPlant"].ToString();
                        plant = dr["ncqPlant"].ToString();
                        txtCustomerRFQ.Text = dr["ncqCustomerRFQ"].ToString();
                        txtProcess.Text = dr["ncqQuoteProcess"].ToString();
                        txtPartNumber.Text = dr["ncqPartNumber"].ToString();
                        txtPartName.Text = dr["ncqProgramName"].ToString();
                        //txtStations.Text = dr["ncqCustomer"].ToString();
                        //txtRFQNumber.Text = dr["ncqCustomer"].ToString();
                        //Double quoteCompany = System.Convert.ToDouble(dr["ncqCustomer"]).ToString());
                        //ddlStatus.SelectedValue = dr["ncqCustomer"].ToString();
                        ddlEstimator.SelectedValue = dr["ncqCreatedBy"].ToString();
                        txtCustomerContact.Text = dr["ncqCustomerContact"].ToString();
                        lblVersion.Text = dr["ncqVersion"].ToString();
                        lblQuoteNumber.Text = dr["ncqQuotationNumber"].ToString();
                        txtProgramKickoff.Text = Convert.ToDateTime(dr["ncqProgramKickOffDate"]).ToString("MM/dd/yyyy");
                        txtProgramPPAP.Text = Convert.ToDateTime(dr["ncqProgramPPAPDate"]).ToString("MM/dd/yyyy");
                        txtProgramSOP.Text = Convert.ToDateTime(dr["ncqProgramSOPDate"]).ToString("MM/dd/yyyy");
                        txtYOP.Text = dr["ncqYearsOfProduction"].ToString();
                        txtVolume.Text = dr["ncqAnnualVolume"].ToString();
                        txtLotSize.Text = dr["ncqQuotedLotSize"].ToString();
                        txtDataRec.Text = Convert.ToDateTime(dr["ncqDataRecievedDate"]).ToString("MM/dd/yyyy");
                        txtDataFileName.Text = dr["ncqDataFileName"].ToString();
                        txtDesignReview.Text = dr["ncqDesignReview"].ToString();
                        txtTryout.Text = dr["ncqTryout"].ToString();
                        txtShippingCompany.Text = dr["ncqShippingCompany"].ToString();
                        txtSetup.Text = dr["ncqSetup"].ToString();
                        txtBuyoff.Text = dr["ncqBuyoff"].ToString();
                        txtPaymentTermsToolingEquipmentCapital.Text = dr["ncqPaymentTermsToolingEquipmentCapital"].ToString();
                        txtPaymentTermsPieceCost.Text = dr["ncqPaymentTermsPieceCost"].ToString();
                        txtRawMaterial.Text = dr["ncqRawMaterial"].ToString();
                        txtWIP.Text = dr["ncqWIP"].ToString();
                        txtFinnishedGoods.Text = dr["ncqFinnishedGoods"].ToString();
                        txtShippingOfFinishedGoods.Text = dr["ncqShippingOfFinishedGoods"].ToString();
                ...
                sql.CommandText = "Select pwnncPreWordedNote, pwnncToolingCost, pwnncCapitalCost, pwnncPieceCost, pwnncPreWordedNoteID from pktblPreWordedNoteNc, linkPWNToNcQuote where pwqncQuoteID = @quoteID and pwqncPreWordedNoteID = pwnncPreWordedNoteID";
                sql.Parameters.Clear();
                sql.Parameters.AddWithValue("@quoteID", quoteID);
        ...
        //                    if (master.getCompanyId() == 1)
        //                    {
        //                        sql.Parameters.AddWithValue("@TSGCompany", "1");
        //                    }
        //                    else
        //                    {
        //                        sql.Parameters.AddWithValue("@TSGCompany", master.getCompanyId());
